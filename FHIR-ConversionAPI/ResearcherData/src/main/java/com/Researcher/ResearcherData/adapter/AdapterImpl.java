...
import com.Researcher.ResearcherData.ValidateFHIR;
import com.Researcher.ResearcherData.domain.model.DTOCsvFormat;
import com.Researcher.ResearcherData.domain.model.DTOJsonFormat;
...
public class AdapterImpl extends ValidateFHIR {
    ...
    public String CSVconverter(MultipartFile multipart) throws IOException, ParseException {
        ...
        String delimiter = ",";
        List<DTOCsvFormat> CSVreceived = new ArrayList<>();
        ...
        while(read.hasNextLine()) {
            ...
            CSVreceived.add(new DTOCsvFormat(userData[0], userData[1],userData[2], userData[3], userData[4], userData[5], userData[6], userData[7]));
        }
        ...
        for ( DTOCsvFormat data: CSVreceived
        ) {
    ...
    public String JSONconverter(String jsonObject) throws IOException {
        ...
        List<DTOJsonFormat> jsoNreceiveds = objectMapper.readValue(jsonObject, new TypeReference<List<DTOJsonFormat>>(){});

        /** POJO to FHIR Format Converter **/

        List<DiagnosticReport> diagnosticReports = new ArrayList<>();
        for ( DTOJsonFormat data: jsoNreceiveds) {
